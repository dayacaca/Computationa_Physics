...
!!$  loop de integración 
  ...
  do l=1,Nt
  t = t + dt
  u_n_p_o=u_n_o  ! 1er reciclado de variables
  
  do i=1,Nx-1 !Calculo la función con lax Friederich en n-1
  u_n_o(i)= 0.5*(u_n_p_o(i+1)+ u_n_p_o(i-1))- 0.5*(dt/dx)*(u_n_p_o(i+1)-u_n_p_o(i-1))
  end do 
 
  u_n_p_o=u_n_o
  u_n_p=u_n !2do reciclado de variable
 
 ! Este paso no estoy segura, ¿La función u_n_o no se, si hace lo que
 ! quiero que haga 
    do i=1,Nx-1
