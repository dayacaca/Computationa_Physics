!Programa que calcula númericamente la ecuación de onda
! con diferencias finitas con el esquema Leapfrog el cual utiliza dos niveles
! de tiempo n y n-1 para calcular la función en un tiempo n+1 
...
!!$ Wave data
  ...
  real(kind=8), allocatable, dimension (:) :: pi_n_o
  real(kind=8), allocatable, dimension (:) :: pi_n_p_o
  
  
  real(kind=8), allocatable, dimension (:) :: psi_n
  real(kind=8), allocatable, dimension (:) :: psi_n_p
  real(kind=8), allocatable, dimension (:) :: psi_n_o
  real(kind=8), allocatable, dimension (:) :: psi_n_p_o
   
  real(kind=8), allocatable, dimension (:) :: phi_n
  real(kind=8), allocatable, dimension (:) :: phi_n_p
 ! real(kind=8), allocatable, dimension (:) :: phi_n_o
 ! real(kind=8), allocatable, dimension (:) :: phi_n_p_o
...
!!$ Allocate memory to the arrays
  ...
  allocate(pi_n_o(0:Nx))  
  allocate(pi_n_p_o(0:Nx)) 
 

  allocate(psi_n(0:Nx))
  allocate(psi_n_p(0:Nx))
  allocate(psi_n_p_o(0:Nx)) 
  allocate(psi_n_o(0:Nx)) 
...
!!$ Initial data
  ...
  pi_n_o = 0d0
  psi_n_o =  -2*amp*((x-x0)/sigma**2) * exp( - ( x - x0 )**2 / sigma**2 ) 
...
!!$ Saving the initial stuff
  ...
  call save1Ddata(Nx,t,x,psi_n_o,'psi_n_o',0)
  call save1Ddata(Nx,t,x,pi_n_o,'pi_n_o',0)


  psi_n_p_o=pi_n
  psi_n_p_o=psi_n
  do i=1,Nx-1 !Calculo la función u_n_o con FTCS en un tiempo fijo
  u_n_o(i)= u_n_p_o(i)- 0.5*(dt/dx)*(u_n_p_o(i+1)-u_n_p_o(i-1))
