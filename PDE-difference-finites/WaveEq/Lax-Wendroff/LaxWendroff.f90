!Programa que calcula númericamente la ecuación de onda dtt phi -v^2 dxx phi=0 
! con C.I y B.C por el método de diferencias finitas utilizando 
!el esquema Lax Wendrof
...
!!$ Integration loop
  ...
  do i=1,Nx-1
		psi_n(i)= psi_n_p(i)+ (dt/dx)*( 0.5*(pi_n_p(i+1)-pi_n_p(i-1)) + 0.5*(dt/dx)*( psi_n_p(i+1)-2*psi_n_p(i) +psi_n_p(i-1)))
		pi_n(i)= pi_n_p(i)+ (dt/dx)*( 0.5*(psi_n_p(i+1)-psi_n_p(i-1)) + 0.5*(dt/dx)*( pi_n_p(i+1)-2*pi_n_p(i) +pi_n_p(i-1)))
  end do
